...
import os
import dotenv
from H5DataModule import H5DataModule
import json
...
def calculate_mean_std(train_loader):
    ...
    for data, _ in train_loader:
        data = data.view(data.size(0), data.size(1), -1)  # Rearrange data for mean/std calculation
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate mean and standard deviation of dataset.") # noqa
    parser.add_argument("--data_file",
                        type=str,
                        default=os.getenv("DATA_FILE"),
                        help="Path to the data file. Defaults to DATA_FILE environment variable.")
    parser.add_argument("--dm_params",
                        type=str,
                        help="DataModule parameters as a JSON string.")
    ...
    if args.dm_params:
        try:
            ...
            dm_params.update(command_line_params)  # Update default params with any provided via cli
