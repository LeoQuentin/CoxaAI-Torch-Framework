...
def preprocess_image(image: torch.Tensor):
    transform_pipeline = transforms.Compose([
        ...
        ImageNetPolicy(),
        # Do not apply ToTensor and Normalize here; ViTFeatureExtractor will handle it
    ])
...
def val_test_preprocess(image: torch.Tensor):
    data = feature_extractor(image=image,
                             return_tensors="pt",
                             input_data_format="channels_first",
                             do_rescale=False)
    pixel_values = data["pixel_values"]
